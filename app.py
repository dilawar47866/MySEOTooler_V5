...
# --- PIPED MIRRORS (The Secret Weapon against YouTube Blocking) ---
PIPED_INSTANCES = [
...
# --- YOUTUBE TO BLOG API (PIPED MIRROR NETWORK) ---
...
def api_youtube_to_blog():
    ...
    try:
        ...
        # 2. PIPED API NETWORK (Bypasses YouTube IP Block)
        # We shuffle the list so we don't hammer one server
        mirrors = PIPED_INSTANCES.copy()
        random.shuffle(mirrors)
        
        for mirror in mirrors:
            try:
                # Request video data from Piped
                ...
                if r.status_code == 200:
                    ...
                    for sub in subtitles:
                        if 'en' in sub.get('code', '') or 'English' in sub.get('name', ''):
                            target_sub = sub
                            break
                    ...
                    # Priority 3: Any Subtitle (We will translate)
                    if not target_sub and len(subtitles) > 0:
                        target_sub = subtitles[0]
                        
                    if target_sub:
                        # Download the VTT/JSON content
                        sub_url = target_sub['url']
                        r_sub = requests.get(sub_url, timeout=5)
                        
                        # Parse JSON format (Piped usually returns JSON/VTT)
                        # Simple regex to extract text if it's VTT-like, or just use raw
                        # Usually cleaning VTT is safer
                        raw_text = r_sub.text
                        
                        # Simple cleaner for VTT/SRT
                        lines = raw_text.splitlines()
                        cleaned = []
                        for line in lines:
                            # Filter out timestamps and empty lines
                            if '-->' not in line and 'WEBVTT' not in line and line.strip() != '' and not line.strip().isdigit():
        ...
        if not full_text or len(full_text) < 50:
            return jsonify({'error': "Could not fetch captions from any server. This video might be completely silent or blocked globally."}), 400
